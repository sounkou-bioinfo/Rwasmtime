...
impl CoreStoreState {
    fn new(limits: CoreExecutionLimits) -> Result<Self> {
        Ok(Self { resource_limits: RuntimeStoreLimits::new(limits)? })
    }
}
...
pub type CoreHostFuncCallback = dyn Fn(&str, &[wasmtime::Val], &mut [wasmtime::Val]) -> wasmtime::Result<()> + Send + Sync + 'static;
...
impl CoreExecutionLimits {
    pub fn none() -> Self {
        Self { memory_bytes: None, table_elements: None, instances: None, fuel: None, wall_time_ms: None }
    }

    pub fn new(fuel: Option<u64>, wall_time_ms: Option<u64>) -> Self {
        Self { fuel, wall_time_ms, ..Self::none() }
    }
...
impl RuntimeStoreLimits {
    fn new(limits: CoreExecutionLimits) -> Result<Self> {
        Ok(Self {
            ...
            instances: optional_limit_usize(limits.instances, "instance")?.unwrap_or(wasmtime::DEFAULT_INSTANCE_LIMIT),
        })
    }
}
    ...
    value
        .map(|value| {
            usize::try_from(value).map_err(|_| {
                RwasmtimeError::invalid_argument(format!("{label} limit exceeds this platform's addressable size"))
            })
        })
        .transpose()
}
...
impl CoreHostFunc {
    pub fn new(
        ...
        Self { module: module.into(), name: name.into(), params, results, callback }
    }
...
impl CoreModule {
    ...
    pub fn instantiate(&self, limits: CoreExecutionLimits) -> Result<CoreInstance> {
        ...
        let instance = wasmtime::Instance::new(&mut store, &self.module, &[])
            .map_err(|err| RwasmtimeError::runtime(format!("failed to instantiate Wasm module: {err:#}")))?;
        Ok(CoreInstance::new_empty(store, instance, limits, wall_clock))
    }

    pub fn instantiate_with_host_funcs(&self, host_funcs: Vec<CoreHostFunc>, limits: CoreExecutionLimits) -> Result<CoreInstance> {
        let mut linker = wasmtime::Linker::new(self.module.engine());
        for host in host_funcs {
            let key = host.key();
            let key_for_callback = key.clone();
            let ty = wasmtime::FuncType::new(self.module.engine(), host.params.clone(), host.results.clone());
            let callback = host.callback.clone();
            linker
                .func_new(&host.module, &host.name, ty, move |_caller, args, results| {
                    callback(&key_for_callback, args, results)
                })
                .map_err(|err| RwasmtimeError::runtime(format!("failed to link callback import `{key}`: {err}")))?;
        }
        let mut store = new_core_store(self.module.engine(), limits)?;
        let wall_clock = configure_store_limits(&mut store, limits)?;
        let _wall_time_guard = enter_wall_time_call(&mut store, &wall_clock);
        let instance = linker
            ...
            .map_err(|err| RwasmtimeError::runtime(format!("failed to instantiate Wasm module with callbacks: {err:#}")))?;
        Ok(CoreInstance::new_empty(store, instance, limits, wall_clock))
    }

    #[cfg(feature = "wasi")]
    pub fn instantiate_wasi_p1(&self, wasi: &WasiSpec, limits: CoreExecutionLimits) -> Result<CoreInstance> {
        wasi.validate()?;
        validate_supported_wasi_backend(wasi)?;

        let mut linker = wasmtime::Linker::new(self.module.engine());
        wasmtime_wasi::p1::add_to_linker_sync(&mut linker, |state: &mut WasiP1State| &mut state.wasi)
        ...
        let instance = linker
            .instantiate(&mut store, &self.module)
            .map_err(|err| RwasmtimeError::runtime(format!("failed to instantiate WASIp1 module: {err:#}")))?;
        Ok(CoreInstance::new_wasi_p1(store, instance, stdout_capture, stderr_capture, limits, wall_clock))
    }

    pub fn serialize(&self) -> Result<Vec<u8>> {
        self.module
            .serialize()
            .map_err(|err| RwasmtimeError::runtime(format!("failed to serialize compiled core module: {err}")))
    }
}
...
impl CoreInstance {
    ...
    fn new_wasi_p1(
        ...
        Self { store: CoreStore::WasiP1(store), instance, limits, wall_clock, stdout_capture, stderr_capture }
    }
    ...
    pub fn call_export(
        ...
        match &mut self.store {
            CoreStore::Empty(store) => call_export_with_limits(&self.instance, store, self.limits, &self.wall_clock, export, args, results),
            #[cfg(feature = "wasi")]
            CoreStore::WasiP1(store) => call_export_with_limits(&self.instance, store, self.limits, &self.wall_clock, export, args, results),
        }
    }
    ...
    pub fn memory_read(&mut self, name: &str, offset: usize, len: usize) -> Result<Vec<u8>> {
        match &mut self.store {
            CoreStore::Empty(store) => memory_read_in_store(&self.instance, store, name, offset, len),
            #[cfg(feature = "wasi")]
            CoreStore::WasiP1(store) => memory_read_in_store(&self.instance, store, name, offset, len),
        }
    }

    pub fn memory_write(&mut self, name: &str, offset: usize, bytes: &[u8]) -> Result<()> {
        match &mut self.store {
            CoreStore::Empty(store) => memory_write_in_store(&self.instance, store, name, offset, bytes),
            #[cfg(feature = "wasi")]
            CoreStore::WasiP1(store) => memory_write_in_store(&self.instance, store, name, offset, bytes),
        }
    }
    ...
    pub fn wasi_output(&self) -> WasiCommandOutput {
        WasiCommandOutput {
            stdout: self.stdout_capture.as_ref().map(|pipe| pipe.contents().to_vec()).unwrap_or_default(),
            stderr: self.stderr_capture.as_ref().map(|pipe| pipe.contents().to_vec()).unwrap_or_default(),
        }
    }
}

fn new_core_store(engine: &wasmtime::Engine, limits: CoreExecutionLimits) -> Result<wasmtime::Store<CoreStoreState>> {
...
#[cfg(feature = "wasi")]
fn new_wasi_p1_store(engine: &wasmtime::Engine, state: WasiP1State) -> Result<wasmtime::Store<WasiP1State>> {
    let mut store = wasmtime::Store::new(engine, state);
    store.limiter(|state| &mut state.resource_limits);
    Ok(store)
}

fn func_type_in_store<T>(instance: &wasmtime::Instance, store: &mut wasmtime::Store<T>, export: &str) -> Result<wasmtime::FuncType> {
    let func = instance
...
fn configure_store_limits<T>(store: &mut wasmtime::Store<T>, limits: CoreExecutionLimits) -> Result<Option<Arc<Mutex<Option<Instant>>>>> {
    let fuel = limits.fuel.unwrap_or(UNLIMITED_FUEL_SENTINEL);
    store
        .set_fuel(fuel)
        .map_err(|err| RwasmtimeError::runtime(format!("failed to configure Wasm fuel limit: {err}")))?;
        ...
        if let Some(ms) = limits.wall_time_ms {
            ...
            store.epoch_deadline_callback(move |_| {
                ...
                if elapsed.is_some_and(|elapsed| elapsed >= limit) {
                    return Err(wasmtime::format_err!("wall time limit exceeded after {ms} ms"));
                }
...
fn enter_wall_time_call<T>(store: &mut wasmtime::Store<T>, wall_clock: &Option<Arc<Mutex<Option<Instant>>>>) -> WallTimeCallGuard {
    let Some(wall_clock) = wall_clock else {
        return WallTimeCallGuard { wall_clock: None, stop: None, handle: None };
    };
    ...
    #[cfg(target_has_atomic = "64")]
    {
        ...
        WallTimeCallGuard { wall_clock: Some(Arc::clone(wall_clock)), stop: Some(stop), handle: Some(handle) }
    }

    #[cfg(not(target_has_atomic = "64"))]
    {
        let _ = store;
        WallTimeCallGuard { wall_clock: None, stop: None, handle: None }
    }
}
...
fn call_export_in_store<T>(
    ...
    func.call(&mut *store, args, results)
        .map_err(|err| RwasmtimeError::runtime(format!("Wasm call `{export}` trapped or failed: {err:#}")))
}

fn memory_in_store<T>(instance: &wasmtime::Instance, store: &mut wasmtime::Store<T>, name: &str) -> Result<wasmtime::Memory> {
    if name.is_empty() {
        return Err(RwasmtimeError::invalid_argument("memory name must not be empty"));
    }
    instance
...
fn memory_size_in_store<T>(instance: &wasmtime::Instance, store: &mut wasmtime::Store<T>, name: &str) -> Result<u64> {
    let memory = memory_in_store(instance, store, name)?;
    Ok(memory.size(&*store))
}

fn memory_grow_in_store<T>(instance: &wasmtime::Instance, store: &mut wasmtime::Store<T>, name: &str, pages: u64) -> Result<u64> {
    let memory = memory_in_store(instance, store, name)?;
    memory
...
fn memory_read_in_store<T>(instance: &wasmtime::Instance, store: &mut wasmtime::Store<T>, name: &str, offset: usize, len: usize) -> Result<Vec<u8>> {
    let memory = memory_in_store(instance, store, name)?;
...
fn memory_write_in_store<T>(instance: &wasmtime::Instance, store: &mut wasmtime::Store<T>, name: &str, offset: usize, bytes: &[u8]) -> Result<()> {
    let memory = memory_in_store(instance, store, name)?;
...
impl WasmtimeRuntime {
    pub fn new(spec: RuntimeSpec) -> Result<Self> {
        ...
        let engine = wasmtime::Engine::new(&config)
            .map_err(|err| RwasmtimeError::runtime(format!("failed to build Wasmtime engine: {err}")))?;
        Ok(Self { spec, engine })
    }
    ...
    pub fn compile_core(&self, module: impl AsRef<[u8]>) -> Result<CoreModule> {
        let module = wasmtime::Module::new(&self.engine, module)
            .map_err(|err| RwasmtimeError::runtime(format!("failed to compile Wasm module: {err}")))?;
    ...
    pub fn deserialize_core(&self, bytes: impl AsRef<[u8]>) -> Result<CoreModule> {
        ...
        let module = unsafe { wasmtime::Module::deserialize(&self.engine, bytes) }
            .map_err(|err| RwasmtimeError::runtime(format!("failed to deserialize compiled core module: {err}")))?;
        Ok(CoreModule { module })
    }
    ...
    fn compile_component_for_introspection(&self, component: impl AsRef<[u8]>) -> Result<wasmtime::component::Component> {
        wasmtime::component::Component::new(&self.engine, component)
            .map_err(|err| RwasmtimeError::runtime(format!("failed to compile Wasm component for introspection: {err:#}")))
    }

    pub fn instantiate_core_module(&self, module: &CoreModule, limits: CoreExecutionLimits) -> Result<CoreInstance> {
    ...
    #[cfg(feature = "wasi")]
    pub fn instantiate_core_module_wasi_p1(&self, module: &CoreModule, wasi: &WasiSpec, limits: CoreExecutionLimits) -> Result<CoreInstance> {
        self.ensure_module_engine(module)?;
    ...
    pub fn instantiate_core(&self, module: impl AsRef<[u8]>, limits: CoreExecutionLimits) -> Result<CoreInstance> {
    ...
    pub fn run_wasi_p1_command(
        ...
        let module = wasmtime::Module::new(&self.engine, module)
            .map_err(|err| RwasmtimeError::runtime(format!("failed to compile WASIp1 module: {err}")))?;
        ...
        let start = instance
            ...
            .map_err(|err| RwasmtimeError::runtime(format!("failed to resolve WASIp1 `_start`: {err}")))?;
        let _wall_time_guard = enter_wall_time_call(&mut store, &wall_clock);
        start
            .call(&mut store, ())
        ...
        Ok(WasiCommandOutput {
            stdout: stdout_capture.map(|pipe| pipe.contents().to_vec()).unwrap_or_default(),
            stderr: stderr_capture.map(|pipe| pipe.contents().to_vec()).unwrap_or_default(),
        })
    }
}
...
fn describe_component_item(name: &str, item: wasmtime::component::types::ComponentItem) -> ComponentItem {
    match item {
        wasmtime::component::types::ComponentItem::ComponentFunc(func) => ComponentItem {
            ...
            results_schema: join_nonempty(func.results().map(|ty| component_type_label(&ty).to_string())),
        },
...
fn build_wasi_p1_state(
    ...
    for preopen in &wasi.preopens {
        ...
        builder
            ...
            .map_err(|err| RwasmtimeError::runtime(format!(
    ...
    Ok((
        WasiP1State { wasi: builder.build_p1(), resource_limits: RuntimeStoreLimits::new(limits)? },
        stdout_capture,
        stderr_capture,
    ))
}
...
impl RwasmtimeError {
    pub fn runtime(message: impl Into<String>) -> Self {
        Self { kind: RwasmtimeErrorKind::Runtime, message: message.into() }
    }
}
...
mod tests {
    ...
    fn cranelift_runtime_compiles_instantiates_and_calls_core_module() {
        ...
        runtime
            .call_core_export(ADD_WAT, "add", &args, &mut results, CoreExecutionLimits::none())
    ...
    fn component_introspection_reports_imports_and_exports() {
        ...
        let imports = runtime.component_imports(COMPONENT_IMPORT_WAT).expect("component imports should parse");
        ...
        let exports = runtime.component_exports(COMPONENT_EXPORT_WAT).expect("component exports should parse");
    ...
    fn core_instance_persists_memory_state_and_grows_memory() {
        ...
        let mut results = [wasmtime::Val::I32(0)];
        instance.call_export("load8", &[wasmtime::Val::I32(2)], &mut results).unwrap();
    ...
    fn store_memory_limit_rejects_initial_memory_above_limit() {
        ...
        assert!(err.message.contains("memory limit exceeded"), "{}", err.message);
    ...
    fn store_table_limit_rejects_initial_table_above_limit() {
        ...
        assert!(err.message.contains("table element limit exceeded"), "{}", err.message);
    ...
    fn store_instance_limit_zero_rejects_instantiation() {
        ...
        assert!(err.message.contains("resource limit exceeded"), "{}", err.message);
    }
    ...
    fn compiled_core_module_instantiates_with_host_callback_imports() {
        ...
        let runtime = RuntimeSpec::new()
            ...
            .build_wasmtime()
            .expect("real Wasmtime runtime should build with Cranelift");
        let module = runtime.compile_core(CALLBACK_WAT).expect("callback module should compile");
        ...
        let mut instance = runtime
            .instantiate_core_module_with_host_funcs(&module, vec![host], CoreExecutionLimits::none())
            .expect("module with host callback should instantiate");
        let mut results = [wasmtime::Val::I32(0)];
        instance.call_export("run", &[wasmtime::Val::I32(1)], &mut results).unwrap();
        assert_eq!(results[0].unwrap_i32(), 42);
    }
    ...
    fn compiled_core_module_can_instantiate_separate_stateful_instances() {
        let runtime = RuntimeSpec::new()
            ...
            .build_wasmtime()
            .expect("real Wasmtime runtime should build with Cranelift");
        let module = runtime.compile_core(MEMORY_WAT).expect("core module should compile once");
        let mut a = runtime.instantiate_core_module(&module, CoreExecutionLimits::none()).expect("first instance");
        let mut b = runtime.instantiate_core_module(&module, CoreExecutionLimits::none()).expect("second instance");
    ...
    fn compiled_core_module_serializes_and_deserializes_with_same_runtime() {
        let runtime = RuntimeSpec::new()
            ...
            .build_wasmtime()
            .expect("real Wasmtime runtime should build with Cranelift");
        let module = runtime.compile_core(ADD_WAT).expect("core module should compile once");
        let bytes = module.serialize().expect("compiled module should serialize");
        assert!(!bytes.is_empty());
        let module = runtime.deserialize_core(&bytes).expect("serialized module should reload");
        let mut instance = runtime.instantiate_core_module(&module, CoreExecutionLimits::none()).expect("deserialized instance");
        let mut results = [wasmtime::Val::I32(0)];
        instance
            .call_export("add", &[wasmtime::Val::I32(2), wasmtime::Val::I32(40)], &mut results)
            .unwrap();
        assert_eq!(results[0].unwrap_i32(), 42);
    }
    ...
    fn compiled_core_module_instantiates_with_wasi_p1_imports() {
        ...
        let runtime = RuntimeSpec::new()
            ...
            .build_wasmtime()
            .expect("real Wasmtime runtime should build for WASI");
        let module = runtime.compile_core(ECHO_STDIN_WASI_WAT).expect("WASI core module should compile");
        ...
        instance.call_export("_start", &[], &mut []).expect("WASIp1 _start should run");
    ...
    fn wasi_preview1_preopen_is_deny_by_default_and_explicit_when_configured() {
        ...
        let denied = runtime
            .run_wasi_p1_command(PREOPEN_PROBE_WASI_WAT, &WasiSpec::new(), CoreExecutionLimits::none())
