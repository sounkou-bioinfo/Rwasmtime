...
persistent session. Rwasmtime can now run the command-style subset of
that artifact in native builds with `exceptions = TRUE`, WASIp1,
`host_result_cap` as a scalar core callback, and `host_call` declared
with `abi = "core_memory_request"`. The persistent session adapter
remains future work because it needs background guest execution and an
explicit request/reply queue, not a host-R shortcut.
